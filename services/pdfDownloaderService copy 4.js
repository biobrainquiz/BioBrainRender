...
exports.downloadResultPdf = async (req, res) => {
    try {
        ...
        result.questions.forEach((q, i) => {
            ...
            options.forEach((opt, j) => {
                ...
                if (isCorrect) {
                    ...
                    if (wasSkipped) {
                        ...
                        // This handles the "Correct Answer" label when the user picked something else
            ...
            doc.fillColor("#7f8c8d").font('Mont-Regular').fontSize(8).text(`Time: ${q.timetaken}s | Marks: ${marksValue >= 0 ? '+' + marksValue : marksValue} | Status: ${statusLabel}`, 55, currentOptY + 4);
            doc.restore();
