...
const generatePdfContent = async (result) => {
    ...
    // ===== Register Fonts (Ensure these paths exist in your project) =====
...
exports.downloadResultPdf = async (req, res) => {
    try {
        const result = await Result.findById(req.params.mocktestid).lean();
        if (!result) return res.status(404).send("Result not found");

        const doc = await generatePdfContent(result);

        res.setHeader("Content-Disposition", `attachment; filename=BioBrain_Report_${result._id}.pdf`);
...
exports.emailResultPdf = async (req, res) => {
    try {
        ...
        // 2. Convert Stream to Buffer (Promisified)
        ...
        const mailOptions = {
            from: `BioBrain <reports@${cleanDomain}>`,
            to: [result.useremail],
        ...
        // 4. Dispatch via Mailer
        const sentInfo = await Mailer.send(mailOptions);
